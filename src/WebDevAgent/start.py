...
"""Start the WebDevAgent - An intelligent HTML code generation and optimization AI system."""
...
def main():
    """Start the WebDevAgent with YAML-based agent configuration."""
    print(f"{Colors.CYAN}{Colors.BOLD}{'=' * 70}")
    print("🌐 WebDevAgent - HTML Code Generation & Optimization System")
    ...
    try:
        ...
        if not api_key or api_key == "your_api_key_here":
            ...
            print(f"{Colors.WHITE}  1. Edit ./.env and set ANTHROPIC_API_KEY")
            print("  2. Export environment variable: export ANTHROPIC_API_KEY=your_key")
        ...
        # Interactive mode
        print(f"{Colors.CYAN}💡 Enter your HTML generation task (or 'exit' to quit){Colors.RESET}")
        ...
        while True:
            ...
            webdev_agent = load_agent_config(str(script_dir / "config.yaml"))

            print(f"\n{Colors.MAGENTA}{Colors.BOLD}╭{'─' * 68}╮{Colors.RESET}")
            print(f"{Colors.MAGENTA}{Colors.BOLD}│ 🤖 WebDevAgent Response{' ' * 44}│{Colors.RESET}")
            print(f"{Colors.MAGENTA}{Colors.BOLD}╰{'─' * 68}╯{Colors.RESET}")
            print()

            response = webdev_agent.run(
                user_message,
                context={
                    "date": get_date(),
            ...
            if webdev_agent.langfuse_trace_id:
                print(f"\n{Colors.BLUE}📊 Langfuse trace ID: {Colors.YELLOW}{webdev_agent.langfuse_trace_id}{Colors.RESET}")
                try:
                    trace_url = langfuse.get_client().get_trace_url(
                        trace_id=webdev_agent.langfuse_trace_id
                    )
        ...
        print(f"\n\n{Colors.YELLOW}👋 WebDevAgent interrupted by user{Colors.RESET}")
        return 0
